...
export const getAllCommentsForVideo = async (req, res, next) => {
    try {
        ...
        const comments = await Comment.find({ video: videoId })
            .populate("author", "username profileImg _id") // Populate author field with user details
            .sort({ createdAt: -1 }); 
            // Sort comments by creation date in descending order or you can say that newest first
